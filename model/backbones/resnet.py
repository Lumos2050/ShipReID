...
class ResNet18(nn.Module):
    ...
    def forward(self, x):
        ...
        x_nobn = x.view(x.size()[0],  -1)
        x_bn = self.bn2(x)

        #print('x', x.shape)

        #print(x.size())
        x_bn = x_bn.view(x_bn.size()[0],  -1)
        #print('ssss', x.shape)
        score = F.relu(self.outlayer(x_bn))
        #x = F.relu(self.outlayer1(x))
        #x = F.relu(self.outlayer2(x))
        #x = F.relu(self.outlayer3(x))
        #x = F.softmax(x, dim=1)
        #print('ssssssssss',x.shape)
        return x_nobn, x_bn, score
...
if __name__ == '__main__':
    ...
    x = torch.randn((5, 256, 64),dtype=torch.float32).cuda()
    x, x_bn, score = model(x)
